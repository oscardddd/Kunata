...
import os
import torch
from torch.distributed.pipelining import pipeline, ScheduleGPipe, SplitPoint, PipelineStage
...
chunks = 4 # Essentially the number of microbatches
microbatch_size = input_tensor.size(0) // chunks  # Calculate microbatch size

# The input tensor after splitting to microbatches
example_input = torch.rand(microbatch_size, input_tensor.size(1)).to(device)

# Initialize pipeline
pipe = pipeline(mn, mb_args=(example_input,), split_spec=split_spec)
...
stage = PipelineStage(pipe, stage_index=rank, num_stages=3, device=device, input_args=(example_input,))
